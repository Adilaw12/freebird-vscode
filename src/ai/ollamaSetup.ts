 ...
 *        b. "Use Cloud Edits" → switch backend to 'cloud' (10 free/day)
...
export async function checkOllamaSetup(context: vscode.ExtensionContext): Promise<void> {
    ...
    } else if (action === 'Use Cloud Edits (10/day free)') {
        await vscode.workspace.getConfiguration('freebird').update('backend', 'cloud', true);
        vscode.window.showInformationMessage(
            'Switched to Freebird Cloud (Gemini Flash, 10 free edits/day). Upgrade to Pro for unlimited.'
        );
